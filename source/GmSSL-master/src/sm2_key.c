...
int sm2_private_key_from_der(SM2_KEY *key, const uint8_t **in, size_t *inlen)
{
	...
	if (asn1_int_from_der(&ver, &d, &dlen) != 1 || asn1_octet_string_from_der(&prikey, &prikey_len, &d, &dlen) != 1 || asn1_explicit_from_der(0, &params, &params_len, &d, &dlen) != 1 || asn1_explicit_from_der(1, &pubkey, &pubkey_len, &d, &dlen) != 1 || asn1_check(ver == EC_private_key_version) != 1 || asn1_length_is_zero(dlen) != 1)
	{
		error_print();
		return -1;
	}
	...
	{
		int curve;
		if (ec_named_curve_from_der(&curve, &params, &params_len) != 1 || asn1_check(curve == OID_sm2) != 1 || asn1_length_is_zero(params_len) != 1)
		{
			error_print();
			return -1;
		}
	...
	{
		SM2_KEY tmp_key;
		if (sm2_public_key_from_der(&tmp_key, &pubkey, &pubkey_len) != 1 || asn1_length_is_zero(pubkey_len) != 1)
		{
			error_print();
			return -1;
		}
...
int sm2_private_key_info_to_der(const SM2_KEY *sm2_key, uint8_t **out, size_t *outlen)
{
	...
	if (asn1_int_to_der(PKCS8_private_key_info_version, NULL, &len) != 1 || sm2_public_key_algor_to_der(NULL, &len) != 1 || asn1_octet_string_to_der(prikey, prikey_len, NULL, &len) != 1 || asn1_sequence_header_to_der(len, out, outlen) != 1 || asn1_int_to_der(PKCS8_private_key_info_version, out, outlen) != 1 || sm2_public_key_algor_to_der(out, outlen) != 1 || asn1_octet_string_to_der(prikey, prikey_len, out, outlen) != 1)
	{
								  ...
								  const uint8_t **in, size_t *inlen)
{
	...
	if (asn1_int_from_der(&version, &d, &dlen) != 1 || sm2_public_key_algor_from_der(&d, &dlen) != 1 || asn1_octet_string_from_der(&prikey, &prikey_len, &d, &dlen) != 1 || asn1_implicit_set_from_der(0, attrs, attrslen, &d, &dlen) < 0 || asn1_length_is_zero(dlen) != 1)
	{
		error_print();
		return -1;
	}
	if (asn1_check(version == PKCS8_private_key_info_version) != 1 || sm2_private_key_from_der(sm2_key, &prikey, &prikey_len) != 1 || asn1_length_is_zero(prikey_len) != 1)
	{
		error_print();
		return -1;
	}
...
int sm2_private_key_info_print(FILE *fp, int fmt, int ind, const char *label, const uint8_t *d, size_t dlen)
{
	...
	if (asn1_octet_string_from_der(&p, &len, &d, &dlen) != 1)
		goto err;
	if (asn1_sequence_from_der(&prikey, &prikey_len, &p, &len) != 1)
		goto err;
	ec_private_key_print(fp, fmt, ind + 4, "privateKey", prikey, prikey_len);
	if (asn1_length_is_zero(len) != 1)
		goto err;
	if ((ret = asn1_implicit_set_from_der(0, &p, &len, &d, &dlen)) < 0)
		goto err;
	else if (ret)
		format_bytes(fp, fmt, ind, "attributes", p, len);
...
int sm2_private_key_info_from_pem(SM2_KEY *sm2_key, FILE *fp)
{
	...
	if (pem_read(fp, "PRIVATE KEY", buf, &len, sizeof(buf)) != 1 || sm2_private_key_info_from_der(sm2_key, &attrs, &attrs_len, &cp, &len) != 1 || asn1_length_is_zero(len) != 1)
	{
		error_print();
		return -1;
	}
...
{
	size_t len = 0;
	if (sm2_public_key_algor_to_der(NULL, &len) != 1 || sm2_public_key_to_der(pub_key, NULL, &len) != 1 || asn1_sequence_header_to_der(len, out, outlen) != 1 || sm2_public_key_algor_to_der(out, outlen) != 1 || sm2_public_key_to_der(pub_key, out, outlen) != 1)
	{
		error_print();
		return -1;
	}
...
int sm2_public_key_info_from_der(SM2_KEY *pub_key, const uint8_t **in, size_t *inlen)
{
	...
	if (sm2_public_key_algor_from_der(&d, &dlen) != 1 || sm2_public_key_from_der(pub_key, &d, &dlen) != 1 || asn1_length_is_zero(dlen) != 1)
	{
		error_print();
		return -1;
	}
...
int sm2_private_key_from_pem(SM2_KEY *a, FILE *fp)
{
	...
	if (sm2_private_key_from_der(a, &cp, &len) != 1 || len > 0)
	{
		error_print();
		return -1;
	}
...
int sm2_public_key_info_from_pem(SM2_KEY *a, FILE *fp)
{
	...
	if (sm2_public_key_info_from_der(a, &cp, &len) != 1 || asn1_length_is_zero(len) != 1)
	{
		error_print();
		return -1;
	}
										...
										uint8_t **out, size_t *outlen)
{
	...
	if (sm2_private_key_info_to_der(sm2_key, &p, &pkey_info_len) != 1 || rand_bytes(salt, sizeof(salt)) != 1 || rand_bytes(iv, sizeof(iv)) != 1 || sm3_pbkdf2(pass, strlen(pass), salt, sizeof(salt), iter, sizeof(key), key) != 1)
	{
		error_print();
		goto end;
	}
										  ...
										  const char *pass, const uint8_t **in, size_t *inlen)
{
	...
	if (pkcs8_enced_private_key_info_from_der(&salt, &saltlen, &iter, &keylen, &prf,
		...
		asn1_check(keylen == -1 || keylen == 16) != 1 || asn1_check(prf == -1 || prf == OID_hmac_sm3) != 1 || asn1_check(cipher == OID_sm4_cbc) != 1 || asn1_check(ivlen == 16) != 1 || asn1_length_le(enced_pkey_info_len, sizeof(pkey_info)) != 1)
	{
		error_print();
		return -1;
	}
	...
	if (sm4_cbc_padding_decrypt(&sm4_key, iv, enced_pkey_info, enced_pkey_info_len,
		...
		sm2_private_key_info_from_der(sm2, attrs, attrs_len, &cp, &pkey_info_len) != 1 || asn1_length_is_zero(pkey_info_len) != 1)
	{
		error_print();
		goto end;
	}
...
int sm2_private_key_info_decrypt_from_pem(SM2_KEY *key, const char *pass, FILE *fp)
{
	...
	if (pem_read(fp, "ENCRYPTED PRIVATE KEY", buf, &len, sizeof(buf)) != 1 || sm2_private_key_info_decrypt_from_der(key, &attrs, &attrs_len, pass, &cp, &len) != 1 || asn1_length_is_zero(len) != 1)
	{
		error_print();
		return -1;
	}
