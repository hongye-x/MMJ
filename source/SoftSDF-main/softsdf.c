	...
	ECCrefPrivateKey *pucPrivateKey)
{
	...
	if (uiAlgID != SGD_SM2_1 && uiAlgID != SGD_SM2_3 && uiAlgID != SGD_SM2)
	{
		error_print();
		return SDR_INARGERR;
	}
	...
	void **phKeyHandle)
{
	...
	if (uiAlgID != SGD_SM2_3 && uiAlgID != SGD_SM2)
	{
		error_print();
		return SDR_INARGERR;
	}
				  ...
				  void **phKeyHandle)
{
	...
	if (keyLen > SOFTSDF_MAX_KEY_SIZE)
	{
		error_print();
		return SDR_INARGERR;
	}
	...
	ECCSignature *pucSignature)
{
	...
	if (uiAlgID != SGD_SM2_1 && uiAlgID != SGD_SM2)
	{
		error_print();
		return SDR_INARGERR;
	}
						 ...
						 ECCSignature *pucSignature)
{
	...
	sm2_z256_from_bytes(sm2_key.private_key, pucPrivateKey->K + ECCref_MAX_LEN - 32);

	if (sm2_do_sign(&sm2_key, pucData, &sig) != 1)
	{
		error_print();
		return SDR_GMSSLERR;
	}
	...
	unsigned int *puiDataLength)
{
	...
	if (sm2_do_decrypt(&sm2_key, &ciphertext, pucData, &plaintext_len) != 1)
	{
		error_print();
		return SDR_GMSSLERR;
	}
	...
	unsigned int uiIDLength)
{
	...
	if (pucPublicKey != NULL)
	{
		...
		memset(&point, 0, sizeof(point));
		memcpy(point.x,&pucPublicKey->x[32],32);
		memcpy(point.y,&pucPublicKey->y[32],32);
		if (sm2_z256_point_from_bytes(&public_key, (uint8_t *)&point) != 1)
		{
			error_print();
			return SDR_INARGERR;
		...
		if (sm2_z256_point_is_on_curve(&public_key) != 1)
		{
			error_print();
			return SDR_INARGERR;
		}
	...
	unsigned int keyIdx)
{
	...
	char filename[256];
	snprintf(filename, sizeof(filename), "kek-%u.key", keyIdx);
	remove(filename);
	return 0;
}